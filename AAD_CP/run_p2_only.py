...
def compile_p2():
    exe = CODES_DIR / 'p2'
    src = CODES_DIR / 'p2.cpp'
    if exe.exists() and os.access(exe, os.X_OK):
        print("p2 executable already exists")
        return True
    if not src.exists():
...
def run():
    # Compile p2
    if not compile_p2():
        print("Failed to compile p2. Exiting.")
        return
    ...
    print(f"\n{'='*60}")
    print(f"Running p2 on {len(files)} test files")
    print(f"{'='*60}\n")
    
    exe_path = CODES_DIR / 'p2'
    ...
    for idx, (category, fpath) in enumerate(files, 1):
        ...
        out_subdir = OUTPUT_DIR / 'p2' / category
        ...
        try:
            ...
            results.append({'algorithm': 'p2', 'category': category, 'file': str(rel), 
    ...
    # Write results CSV
    csv_path = OUTPUT_DIR / 'p2_results.csv'
    ...
    print(f"\n{'='*60}")
    print("p2 testing complete!")
    print(f"Results saved to: {csv_path}")
    print(f"Output files in: {OUTPUT_DIR / 'p2'}")
    print(f"{'='*60}\n")
