...
def compile_p5():
    exe = CODES_DIR / 'p5'
    src = CODES_DIR / 'p5.cpp'
    if exe.exists() and os.access(exe, os.X_OK):
        print("p5 executable already exists")
        return True
    if not src.exists():
...
def run():
    # Compile p5
    if not compile_p5():
        print("Failed to compile p5. Exiting.")
        return
    ...
    print(f"\n{'='*60}")
    print(f"Running p5 on {len(files)} test files")
    print(f"{'='*60}\n")
    
    exe_path = CODES_DIR / 'p5'
    ...
    for idx, (category, fpath) in enumerate(files, 1):
        ...
        out_subdir = OUTPUT_DIR / 'p5' / category
        ...
        try:
            ...
            results.append({'algorithm': 'p5', 'category': category, 'file': str(rel), 
    ...
    # Write results CSV
    csv_path = OUTPUT_DIR / 'p5_results.csv'
    ...
    print(f"\n{'='*60}")
    print("p5 testing complete!")
    print(f"Results saved to: {csv_path}")
    print(f"Output files in: {OUTPUT_DIR / 'p5'}")
    print(f"{'='*60}\n")
