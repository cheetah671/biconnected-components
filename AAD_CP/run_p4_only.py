...
def compile_p4():
    exe = CODES_DIR / 'p4'
    src = CODES_DIR / 'p4.cpp'
    if exe.exists() and os.access(exe, os.X_OK):
        print("p4 executable already exists")
        return True
    if not src.exists():
...
def run():
    # Compile p4
    if not compile_p4():
        print("Failed to compile p4. Exiting.")
        return
    ...
    print(f"\n{'='*60}")
    print(f"Running p4 on {len(files)} test files")
    print(f"{'='*60}\n")
    
    exe_path = CODES_DIR / 'p4'
    ...
    for idx, (category, fpath) in enumerate(files, 1):
        ...
        out_subdir = OUTPUT_DIR / 'p4' / category
        ...
        try:
            ...
            results.append({'algorithm': 'p4', 'category': category, 'file': str(rel), 
    ...
    # Write results CSV
    csv_path = OUTPUT_DIR / 'p4_results.csv'
    ...
    print(f"\n{'='*60}")
    print("p4 testing complete!")
    print(f"Results saved to: {csv_path}")
    print(f"Output files in: {OUTPUT_DIR / 'p4'}")
    print(f"{'='*60}\n")
