...
def compile_p3():
    exe = CODES_DIR / 'p3'
    src = CODES_DIR / 'p3.cpp'
    if exe.exists() and os.access(exe, os.X_OK):
        print("p3 executable already exists")
        return True
    if not src.exists():
        print(f"Source file {src} not found!")
        return False
    print(f"Compiling {src} -> {exe} ...")
    r = subprocess.run(['g++', str(src), '-O2', '-std=c++17', '-fopenmp', '-o', str(exe)], 
...
def run():
    # Compile p3
    if not compile_p3():
        print("Failed to compile p3. Exiting.")
        return
    ...
    print(f"\n{'='*60}")
    print(f"Running p3 on {len(files)} test files")
    print(f"{'='*60}\n")
    
    exe_path = CODES_DIR / 'p3'
    ...
    for idx, (category, fpath) in enumerate(files, 1):
        ...
        out_subdir = OUTPUT_DIR / 'p3' / category
        ...
        try:
            ...
            results.append({'algorithm': 'p3', 'category': category, 'file': str(rel), 
    ...
    # Write results CSV
    csv_path = OUTPUT_DIR / 'p3_results.csv'
    ...
    print(f"\n{'='*60}")
    print("p3 testing complete!")
    print(f"Results saved to: {csv_path}")
    print(f"Output files in: {OUTPUT_DIR / 'p3'}")
    print(f"{'='*60}\n")
